...
public class ChainedFilterTest extends TestCase {
    ...
    public void setUp() throws Exception {
        ...
        for (int i = 0; i < MAX; i++) {
            ...
            doc.add(new Field(
                    "date",
                    DateTools.timeToString(cal.getTimeInMillis(), DateTools.Resolution.DAY),
                    Field.Store.YES,
                    Field.Index.NOT_ANALYZED));
        ...
        dateFilter = TermRangeFilter.Less(
                "date",                                                                   //C
                DateTools.timeToString(cal.getTimeInMillis(), DateTools.Resolution.DAY)); //C

        bobFilter = new CachingWrapperFilter(                                     //D
                new QueryWrapperFilter(new TermQuery(new Term("owner", "bob")))); //D

        sueFilter = new CachingWrapperFilter(                                     //E
                new QueryWrapperFilter(new TermQuery(new Term("owner", "sue")))); //E
    }
/*
#A 2009 January 01
